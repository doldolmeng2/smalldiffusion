...
n_per_class=0 은 특별 규약: 원본 train 60,000장 전체(불균형 그대로)를 저장한다.
(MNIST는 클래스당 5421~6742장으로 불균형이라 '클래스당 6000장 균등'은 불가능)

사용 예 (src/src 에서):
    python experiments/make_real_subset.py --n-per-class 2000
    python experiments/make_real_subset.py --n-per-class 0      # train 60k 전체
"""
import argparse
import sys
...
def make_real_subset(n_per_class: int, seed: int = 0, out_dir=None) -> Path:
    ...
    # n_per_class=0 → 원본 train 전체 (클래스 불균형 그대로, 추출 없음)
    if n_per_class == 0:
        images01 = data.float() / 255.0
        meta = dict(kind='real', cfg_scale=None, n_per_class=0,
                    total=len(targets), seed=seed,
                    source='MNIST train FULL 60k (no flip, unbalanced)')
        C.save_dataset(out_dir, images01, targets, meta)
        print(f'[real_subset] 저장 완료: {out_dir} ({len(targets)}장, train 전체)')
        return out_dir
